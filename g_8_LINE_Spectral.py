...
# LINE - first-order + second-order proximity

# Spectral (Laplacian eigenvectors)
# Laplacian Eigenmaps – zapravo spektralni embedding

# GRASP + kvantna regresija, deterministički, strukturno
# Varijanta: LINE embedding (first-order proximity, SVD aproksimacija)

"""
Graphs in Space: Graph Embeddings for Machine Learning on Complex Data
LINE = first-order (i second-order) proximity; ovde first-order preko SVD normalizovane matrice
"""
...
P = red-normalizovana matrica susedstva 
(P_ij = M_ij / sum_j M_ij), first-order proximity.

SVD(P) = U, S, Vh; 
embedding = U[:, :k] * sqrt(S[:k]), 
...
EMBED_DIM = 3    # dimenzija embeddinga
MAX_EPOCHS = 20  # dovoljan broj iteracija
LR = 0.2         # learning rate (step size)
FD_EPS = 1e-3    # finite difference epsilon (za numeričku derivaciju)
...
def compute_line_embeddings(M, k=EMBED_DIM):
...
def main():
    ...
    emb = compute_line_embeddings(M, k=EMBED_DIM)
    ...
    print()
    print("Predikcija (LINE + kvantna regresija, deterministički, strukturno):")
    print(best_combo)
    print()
    """
    redikcija (LINE + kvantna regresija, deterministički, strukturno):
    (8, 11, x, y, z, 28, 33)
    """
